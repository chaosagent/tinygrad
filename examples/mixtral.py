...
from tinygrad import Tensor, nn, Device, GlobalCounters, Variable, TinyJit
...
class MixtureFeedForward:
  def __init__(self, num_experts:int, dim:int, hidden_dim:int, linear=nn.Linear):
    ...
    self.jits = [TinyJit(expert) for expert in self.experts]
  def __call__(self, x:Tensor) -> Tensor:
    ...
    e1, e2 = self.jits[top[0][0]], self.jits[top[1][0]]
    scale = Tensor([top[0][1]/norm, top[1][1]/norm], device=x.device)
    ret = e1(x) * scale[0] + \
          e2(x) * scale[1]
    return ret
...
if __name__ == "__main__":
  ...
  for k in (t := tqdm(state, disable=CI)):
    ...
    if isinstance(device, tuple):
      if 'scale' in k: axis = None
      elif '.attention.' in k: axis = -1
      elif '.feed_forward.' in k: axis = -1
      elif 'tok_embeddings.weight' in k: axis = -1
      elif 'output.weight' in k: axis = -1
      else: axis = None
      model_state_dict[k].replace(state[k].to(Device.DEFAULT).shard(device, axis=axis).half()).realize()
    else:
  ...
  for k, v in get_state_dict(model).items():
    if not isinstance(v.device, tuple):
      print(k)
      v.replace(v.shard(device)).realize()
  ...
  for i in range(args.count):
    ...
    with Profiling(sort="time", frac=0.1, enabled=args.profile):
      with Timing("total ", enabled=args.timing, on_exit=lambda x: f", {1e9/x:.2f} tok/sec"):
        tok = model(Tensor([toks[start_pos:]], device=device), 0 if start_pos == 0 else Variable("start_pos", 1, 1024).bind(start_pos), args.temperature).item()
    toks.append(tok)
    start_pos += 1
