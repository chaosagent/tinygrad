...
class Conv2dHeNormal(nn.Conv2d):
  def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True):
    ...
    if getenv("KRSC", 1): self.weight = self.weight.permute(0, 2, 3, 1).contiguous()
    if bias: self.bias = self.bias.cast(dtypes.float32)
  def __call__(self, x: Tensor):
    return x.conv2d(self.weight.cast(dtypes.default_float).permute(*([0, 3, 1, 2] if getenv("KRSC", 1) else [0, 1, 2, 3])), self.bias.cast(dtypes.default_float) if self.bias is not None else None,
