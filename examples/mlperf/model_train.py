...
def train_resnet():
  ...
  def sparse_categorical_crossentropy(out, Y, label_smoothing=0.0):
    sce = out.sparse_categorial_crossentropy(Y)
    return (1 - label_smoothing) * sce + (-label_smoothing * out.mean())
