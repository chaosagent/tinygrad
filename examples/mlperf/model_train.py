...
def train_resnet():
  ...
  scheduler = CosineAnnealingLR(optimizer, epochs*steps_in_train_epoch)
  ...
  for e in range(epochs):
    ...
    for i, (X, Y, data_time) in enumerate(tqdm(t := cross_process(lambda: iterate(bs=BS, val=False, num_workers=16)), total=steps_in_train_epoch)):
      ...
      tqdm.write(f"{i:5} {((dte-dt)+(cl-st))*1000.0:7.2f} ms run, {(et-st)*1000.0:7.2f} ms python, {(cl-et)*1000.0:7.2f} ms CL, {(dte-dt)*1000.0:7.2f} ms fetch data, {loss_cpu:7.2f} loss, {GlobalCounters.mem_used/1e9:.2f} GB used, {GlobalCounters.global_ops*1e-9/(cl-st):9.2f} GFLOPS")
    ...
    if e % 4 == 1:
      ...
      for (X, Y, data_time) in (t := tqdm(cross_process(lambda: iterate(bs=BS, val=True, num_workers=16)), total=steps_in_val_epoch)):
