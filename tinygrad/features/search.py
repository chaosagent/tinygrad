...
actions += [Opt(op=OptOps.LOCAL, axis=axis, amt=amt) for amt in [2,3,4,8,13,16,19,29] for axis in range(5)]
...
actions += [Opt(op=OptOps.TC, axis=axis, amt=1) for axis in range(20)]
...
def beam_search(lin:Linearizer, rawbufs, amt:int, allow_test_size=True) -> Linearizer:
  ...
  if (val:=diskcache_get("beam_search", key)) is not None and not getenv("IGNORE_BEAM_CACHE") and CACHELEVEL >= 2:
    ret = lin.copy()
  ...
  default_parallel, min_progress_micros = multiprocessing.cpu_count() if lin.opts.device in {"CUDA", "HSA", "KFD"} else 0, getenv("BEAM_MIN_PROGRESS",0.01)
  if beam_pool is None and getenv("PARALLEL", default_parallel):
    beam_pool = multiprocessing.get_context("spawn").Pool(getenv("PARALLEL", default_parallel), _init_worker, (), getenv("BEAM_MAX_TASKS_PER_CHILD", 16))
