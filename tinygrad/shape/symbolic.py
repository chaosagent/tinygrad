...
class Node:
  ...
  def expand(self, idxs:Optional[Tuple[VariableOrNum, ...]]=None) -> List[Node]:
    ...
    return [self.substitute(dict(zip(idxs, (Variable.num(x) for x in rep)))) for rep in Node.iter_idxs(idxs)]
  @staticmethod
  ...
  def __mul__(self, b:Union[Node, int]):
    ...
    if b == 1: return self
    if self.__class__ is NumNode: return Variable.num(self.b*b) if isinstance(b, int) else b*self.b
  ...
  def __rfloordiv__(self, b:int):
    if self.min > b >= 0: return Variable.num(0)
    if isinstance(self, NumNode): return Variable.num(b // self.b)
    raise RuntimeError(f"not supported: {b} // {self}")
  def __floordiv__(self, b:Union[Node,int], factoring_allowed=True):
    if isinstance(b, Node):
      if b.__class__ is NumNode: return self // b.b
      if self == b: return Variable.num(1)
      if (b - self).min > 0 and self.min >= 0: return Variable.num(0) # b - self simplifies the node
  ...
  def __rmod__(self, b:int):
    if self.min > b >= 0: return Variable.num(b)
    if isinstance(self, NumNode): return Variable.num(b % self.b)
    raise RuntimeError(f"not supported: {b} % {self}")
  def __mod__(self, b:Union[Node,int]):
    if isinstance(b, Node):
      if b.__class__ is NumNode: return self % b.b
      if self == b: return Variable.num(0)
  ...
  def sum(nodes:List[Node]) -> Node:
    ...
    if not nodes: return Variable.num(0)
    ...
    if num_node_sum: new_nodes.append(Variable.num(num_node_sum))
    return create_rednode(SumNode, new_nodes) if len(new_nodes) > 1 else new_nodes[0] if len(new_nodes) == 1 else Variable.num(0)

  @staticmethod
  def ands(nodes:List[Node]) -> Node:
    if not nodes: return Variable.num(1)
    if len(nodes) == 1: return nodes[0]
    if any(not x for x in nodes): return Variable.num(0)

    # filter 1s
    nodes = [x for x in nodes if x.min != x.max]
    return create_rednode(AndNode, nodes) if len(nodes) > 1 else (nodes[0] if len(nodes) == 1 else Variable.num(1))
...
class Variable(Node):
  def __new__(cls, expr:str, nmin:int, nmax:int, expand=False):
    ...
    if nmin == nmax: return Variable.num(nmin)
...
def create_node(ret:Node):
  ...
  if ret.min == ret.max: return Variable.num(ret.min)
  return ret
...
class SumNode(RedNode):
  ...
  def __floordiv__(self, b: Union[Node, int], factoring_allowed=True):
    ...
    if isinstance(b, SumNode):
      ...
      if nu_num > 0 and de_num and (d:=nu_num//de_num) > 0: return Variable.num(d) + (self-b*d) // b
    if isinstance(b, Node):
...
class AndNode(RedNode):
  ...
  def substitute(self, var_vals: Dict[VariableOrNum, Node]) -> Node:
    ...
    for node in self.nodes:
      if not (sub:=node.substitute(var_vals)): return Variable.num(0)
      subed.append(sub)
