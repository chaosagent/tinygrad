...
class Tensor:
  ...
  def _pool(self, k_:Tuple[sint, ...], stride:Union[Tuple[int, ...], int]=1, dilation:Union[Tuple[int, ...], int]=1, extra_expand=(None, None)) -> Tensor:
    ...
    xup = self.repeat([(extra_expand[1] if i == extra_expand[0] else 1) for i in range(len(self.shape))]).pad(tuple(noop_ + [(0, max(0,o*s-i)) for i,o,s in zip(i_, o_, s_)])).shrink(tuple(noop_ + [(0,o*s) for o,s in zip(o_, s_)]))
  ...
  def conv2d(self, weight:Tensor, bias:Optional[Tensor]=None, groups=1, stride=1, dilation=1, padding=0, acc_dtype:Optional[DType]=None) -> Tensor:
    ...
    rcout = cout//groups
    x = self.reshape(bs, cin_, 1, *self.shape[2:]).pad2d(padding_)._pool(HW, stride, dilation, extra_expand=(2, rcout))   # (bs, groups*cin, oy, ox, H, W)
    ...
    if not all(x == 3 for x in HW) or stride != 1 or dilation != 1 or not WINO:
      ...
      ret = (x * weight.reshape(1, groups, rcout, *[1] * len(oyx), cin, *HW).expand(bs, groups, rcout, *[1]*len(oyx), cin, *HW)) \
