...
from tinygrad.ops import LoadOps, ScheduleItem, BufferOps, LazyOp, ReduceOps, ConstBuffer, MemBuffer, UNSAFE_PAD_OPS, UnaryOps, BinaryOps
...
def _recursive_lazyop(buf:LazyBuffer, inputs:List[LazyBuffer], outbufs:Tuple[LazyBuffer, ...], var_vals:Dict[Variable, int], st:ShapeTracker,
  ...
  if buf.op is BinaryOps.MUL and getenv("MUL_MASK_OPT", 1):
    src_ops = tuple(_recursive_lazyop(x, inputs, outbufs, var_vals, ShapeTracker.from_shape(st.shape), realizes, cache, assign_to=assign_to, assign_idx=assign_idx) for x in buf.srcs)
    if not any(lop.op in ReduceOps for src_op in src_ops for lop in src_op.lazyops):
      ...
      final_src_ops = tuple(_recursive_lazyop(x, inputs, outbufs, var_vals, mangled_sts[src_i], realizes, cache, assign_to=assign_to, assign_idx=assign_idx) for src_i, x in enumerate(buf.srcs))
      cache[(buf, st)] = ret = LazyOp(buf.op, final_src_ops, buf.arg)
...
def _recurse_lb(buf:LazyBuffer, realizes:Dict[LazyBuffer, None], allbufs:Dict[LazyBuffer, None],
  ...
  if buf.base != buf:
    ...
    if (base_sz := prod(buf.base.st.shape)) < prod(buf.st.shape):
      if all_int(buf.base.st.shape) and all(all_int(view.shape) for view in buf.st.views):
