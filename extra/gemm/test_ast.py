...
opts = [Opt(OptOps.UPCAST, 2, 2), Opt(OptOps.UPCAST, 3, 2), Opt(OptOps.UNROLL, 2, 3), Opt(OptOps.UNROLL, 1, 3), Opt(OptOps.TC, 0, 2)]
opts += [Opt(OptOps.UPCAST, 1, 4), Opt(OptOps.UPCAST, 3, 4)]
#opts = [Opt(OptOps.TC, 0, 2)]
#opts = [Opt(op=OptOps.TC, axis=2, amt=2), Opt(op=OptOps.UPCAST, axis=3, amt=4), Opt(op=OptOps.UPCAST, axis=1, amt=4)]
...
prog = ExecItem(Device[Device.DEFAULT].to_program(lin), bufs)
